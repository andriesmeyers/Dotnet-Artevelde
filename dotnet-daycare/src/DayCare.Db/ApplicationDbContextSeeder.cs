            ...
            using(var context = serviceProvider.GetService<ApplicationDbContext>()) {
                ...
                if(!context.Tags.Any())
                {
                    ...
                    var tagSkeleton = new Faker<DayCare.Models.Tag>()
                        .RuleFor(t => t.Name, f => lorem.Word())
                        .RuleFor(t => t.Description, f => String.Join(" ", lorem.Words(5)))
                        .FinishWith((f, u) =>
                        {
                ...
                {
                    var lorem = new Bogus.DataSets.Lorem(locale: "nl");
                    var activitytypeSkeleton = new Faker<DayCare.Models.ActivityType>()
                        .RuleFor(a => a.Name, f => lorem.Word())
                        .RuleFor(a => a.Description, f => String.Join(" ", lorem.Words(5)))
                        .RuleFor(a => a.ThumbnailURL, f => String.Join(" ", lorem.Words(5)))
                        .FinishWith((f, u) =>
                        {
                            Console.WriteLine("ActivityTypes created with Bogus: {0}!", u.Name);
                        });
                    ...
                    for(var i = 0; i < 10;i++)
                    {
                        var activitytype = activitytypeSkeleton.Generate();
